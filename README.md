...
- Сворачивание в системный трей для фоновой работы.
...
- Пункт меню: `Файл -> Свернуть в трей`.
- При закрытии окна по `X` приложение не завершается, а уходит в трей.
- Для полного завершения используйте:
  - `Файл -> Выход` из главного окна, или
  - `Выход` в контекстном меню иконки трея.
- Режим запуска в трей через аргумент: `--start-in-tray`.
- Принудительный запуск с окном: `--start-visible`.
...
- Закрытие окна (`X`) — сворачивает приложение в трей.
...
### Автозапуск сразу в трей

Для Windows-автозагрузки укажите команду:
`"C:\Path\To\TextPaster.exe" --start-in-tray`

Если нужно запустить обычным способом (с окном), используйте:
`"C:\Path\To\TextPaster.exe" --start-visible`
